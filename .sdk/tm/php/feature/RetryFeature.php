...
// QuoteRetrieval SDK retry feature
...
class QuoteRetrievalRetryFeature extends QuoteRetrievalBaseFeature
{
    ...
    public function init(QuoteRetrievalContext $ctx, array $options): void
    {
        ...
        $utility->fetcher = function (QuoteRetrievalContext $ctx2, string $url, array $fetchdef) use ($inner): array {
            return $this->_with_retry($ctx2, $url, $fetchdef, $inner);
        };
    }

    private function _with_retry(QuoteRetrievalContext $ctx, string $url, array $fetchdef, callable $inner): array
    {
        $opts = $this->options ?? [];
    ...
    private function _track(QuoteRetrievalContext $_ctx, int $attempt, mixed $res, mixed $err, float $wait): void
    {
        $client = $this->client;
