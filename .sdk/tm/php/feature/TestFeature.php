...
// QuoteRetrieval SDK test feature

require_once __DIR__ . '/BaseFeature.php';
require_once __DIR__ . '/../utility/Param.php';

class QuoteRetrievalTestFeature extends QuoteRetrievalBaseFeature
{
    ...
    public function init(QuoteRetrievalContext $ctx, array $options): void
    {
        ...
        $test_fetcher = function (QuoteRetrievalContext $fctx, string $_fullurl, array $_fetchdef) use ($entity): array {
            ...
            if ($op->name === 'load') {
                ...
                $id = QuoteRetrievalParam::call($fctx, 'id');
    ...
    public function makeNetsim(array $net, callable $inner): callable
    {
        ...
        return function (QuoteRetrievalContext $fctx, string $url, array $fetchdef) use ($net, $inner, $pick_latency, $sleep): array {
            $this->_netcalls++;
            $call = $this->_netcalls;
     ...
     */
    public function buildArgs(QuoteRetrievalContext $ctx, $op, $args): array
    {
        ...
        foreach ($keys as $k) {
            ...
            if ($is_id || $in_reqd) {
                $v = QuoteRetrievalParam::call($ctx, $k);
