...
// QuoteRetrieval SDK feature test
//
...
class FtCtrl extends QuoteRetrievalControl
{
...
class FtClient
{
    ...
    private QuoteRetrievalUtility $utility;

    public function __construct(QuoteRetrievalUtility $utility, array $options)
    {
    ...
    public function get_utility(): QuoteRetrievalUtility
    {
        return $this->utility;
    }
}
...
class FtRecorder
{
    ...
    {
        return function (QuoteRetrievalContext $_ctx, string $url, array $fetchdef): array {
...
class FtHarness
{
    ...
    public QuoteRetrievalContext $rootctx;
    ...
    public function __construct(
        ...
        $utility->fetcher = $server ?? function (QuoteRetrievalContext $_ctx, string $_url, array $fetchdef): array {
        ...
        foreach ($features as $fspec) {
            ...
            $f = QuoteRetrievalConfig::make_feature($name);
    ...
    private static function buildUrl(QuoteRetrievalSpec $spec): string
    {
        $keys = [];
    ...
    public function op(array $o = []): array
    {
        ...
        if (($ctx->out['point'] ?? null) instanceof QuoteRetrievalError) {
        ...
        if (is_array($fetched)) {
            $ctx->response = new QuoteRetrievalResponse($fetched);
        }
    ...
    private function fail(QuoteRetrievalContext $ctx, mixed $err): array
    {
...
class FeatureTest extends TestCase
{
    ...
    public function test_retry_retries_a_transport_error_until_exhausted(): void
    {
        ...
        $clock = new FtClock();
        $n = 0;
        $server = function (QuoteRetrievalContext $ctx, string $_url, array $_fd) use (&$n): array {
            $n++;
