...
class QuoteRetrievalContext:
    def __init__(self, ctxmap=None, basectx=None):
        ...
        # Ctrl
        self.ctrl = QuoteRetrievalControl()
        ...
        if isinstance(sp, QuoteRetrievalSpec):
            self.spec = sp
        elif basectx is not None:
        ...
        if isinstance(r, QuoteRetrievalResult):
            self.result = r
        elif basectx is not None:
        ...
        if isinstance(rp, QuoteRetrievalResponse):
            self.response = rp
        elif basectx is not None:
    ...
    def resolve_op(self, opname):
        ...
        if opname == "":
            return QuoteRetrievalOperation({})
    ...
    def make_error(self, code, msg):
        return QuoteRetrievalError(code, msg, self)
