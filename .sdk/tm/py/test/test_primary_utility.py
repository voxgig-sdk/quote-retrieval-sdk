...
from projectname_sdk import QuoteRetrievalSDK
...
def _err_from_map(m):
    ...
    if msg == "":
        return None
    code = m.get("code", "")
    return QuoteRetrievalError(code, msg)
...
class TestPrimaryUtility:
    ...
    def test_make_error_no_throw(self):
        ...
        ctx.result = QuoteRetrievalResult({
            "ok": False,
            "resdata": {"id": "safe01"},
        })
    ...
    def test_feature_init_basic(self):
        init_client = QuoteRetrievalSDK.test(None, None)
    ...
    def test_feature_init_inactive(self):
        ...
        class TestInitFeatureInactive(QuoteRetrievalBaseFeature):
            def __init__(self):
                super().__init__()
                self.name = "nofeat"
                self.active = False
    ...
    def test_fetcher_blocked_test_mode(self):
        ...
        blocked_client = QuoteRetrievalSDK({
            "system": {
                "fetch": mock_fetch,
            },
        })
    ...
    def test_make_result_no_result(self):
        ...
        utility = client._utility
        ctx = _make_test_full_ctx(client, utility)
        ctx.spec = QuoteRetrievalSpec({"step": "start"})
        ctx.result = None
    ...
    def test_make_spec_basic(self):
        ...
        spec_client = QuoteRetrievalSDK.test(None, setup_opts)
    ...
    def test_operator_basic(self):
        ...
        def subject(entry):
            ...
            if not isinstance(in_val, dict):
                in_val = {}
            op = QuoteRetrievalOperation(in_val)
            return {
                "entity": op.entity,
                "name": op.name,
                "input": op.input,
    ...
    def test_prepare_auth_basic(self):
        ...
        auth_client = QuoteRetrievalSDK.test(None, setup_opts)
