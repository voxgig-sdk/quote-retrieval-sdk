...
AQL-driven command-line client **and** interactive REPL for the QuoteRetrieval
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/quote-retrieval-cli)
make build

# 2. See usage (words, entities, env vars)
./quote-retrieval-cli --help
...
./quote-retrieval-cli list quote
./quote-retrieval-cli load 1 quote            # {id:1} shorthand
./quote-retrieval-cli load '{id:1}' quote       # explicit match map

# 5. Override the API base URL for a single call
QUOTE_RETRIEVAL_BASE=https://api.example.com ./quote-retrieval-cli list quote

# 6. No arguments -> interactive REPL
./quote-retrieval-cli
quote-retrieval> list quote
quote-retrieval> /quit
```
...
1. **Build the binary.** From this `go-cli/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/quote-retrieval-cli
   ```
...
3. **Run a query.** Evaluate an AQL expression against the API (or run with no
   ...
   ```sh
   ./dist/*/quote-retrieval-cli list quote
   ```
...
```sh
./quote-retrieval-cli load 1 quote          # scalar shorthand for {id:1}
./quote-retrieval-cli load '{id:1}' quote     # explicit match map
```
...
Run with no arguments to open a REPL (prompt `quote-retrieval>`). Each line is
evaluated as its own AQL expression:

```text
$ ./quote-retrieval-cli
quote-retrieval> list quote
quote-retrieval> /help
quote-retrieval> /quit
```
...
| Target | Result |
|--------|--------|
| `make build` | Native binary at `dist/<os>-<arch>/quote-retrieval-cli`. |
...
(`./quote-retrieval-cli <expr>`) and interactively (the REPL), and expressions compose the
