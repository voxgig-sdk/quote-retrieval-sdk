// Command QuoteRetrieval-cli is a CLI + REPL for the QuoteRetrieval SDK,
...
func usage(out io.Writer) {
	...
	fmt.Fprintln(out, "Env:      QUOTE_RETRIEVAL_APIKEY (api key), QUOTE_RETRIEVAL_BASE (base url override)")
...
func run(args []string, in io.Reader, out, errOut io.Writer) int {
	...
	// Configure from the environment: QUOTE_RETRIEVAL_APIKEY carries the API key and
	// QUOTE_RETRIEVAL_BASE optionally overrides the API base URL (e.g. production).
	// Both injectable by a secrets vault. Unset -> nil config defaults.
	var opts map[string]any
	if apikey := os.Getenv("QUOTE_RETRIEVAL_APIKEY"); apikey != "" {
		opts = map[string]any{"apikey": apikey}
	}
	if base := os.Getenv("QUOTE_RETRIEVAL_BASE"); base != "" {
		if opts == nil {
			opts = map[string]any{}
		}
		opts["base"] = base
	}
	client := sdk.NewQuoteRetrievalSDK(opts)
