...
[MCP](https://modelcontextprotocol.io) server exposing the QuoteRetrieval SDK as
two agent tools — `quote-retrieval_list` and `quote-retrieval_load` — built on the
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/quote-retrieval-mcp)
make build

# 2. Provide credentials via the environment
export QUOTE_RETRIEVAL_APIKEY=sk_live_xxx

# 3a. Install into Claude Code over stdio (most common)
claude mcp add --scope user quote-retrieval \
  -- /absolute/path/to/quote-retrieval-mcp -transport stdio

# 3b. …or run a shared HTTP server instead
./quote-retrieval-mcp -transport http -addr :8080
```

Tool-call arguments (what an agent sends):

```jsonc
// quote-retrieval_list: first page of records
{ "entity": "quote" }
{ "entity": "quote", "query": { } }

// quote-retrieval_load: one record by id
...
1. **Build** the server from this `go-mcp/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/quote-retrieval-mcp
   ```
...
3. **Install it into Claude Code** (stdio transport):
   ...
   ```sh
   claude mcp add --scope user quote-retrieval \
     -- "$PWD"/dist/*/quote-retrieval-mcp -transport stdio
   ```

4. **Restart Claude Code.** The `quote-retrieval_list` and `quote-retrieval_load` tools now appear
   in new sessions. Ask the agent to *"list quote using quote-retrieval"*
   and it calls `quote-retrieval_list` with `{"entity":"quote"}`.
...
```sh
export QUOTE_RETRIEVAL_APIKEY=sk_live_xxx            # API key
export QUOTE_RETRIEVAL_BASE=https://api.example.com  # optional: override the API base URL
```
...
### Call the `quote-retrieval_list` tool
...
### Call the `quote-retrieval_load` tool
...
| Tool | Args | Returns |
|------|------|---------|
| `quote-retrieval_list` | `entity` (required), `query` (optional map) | First page of records as JSON |
| `quote-retrieval_load` | `entity` (required), `query` = `{id:N}` | Single record as JSON |
...
| Variable | Purpose |
|----------|---------|
| `QUOTE_RETRIEVAL_APIKEY` | API key sent with every request. |
| `QUOTE_RETRIEVAL_BASE` | Optional override of the API base URL. |
...
```sh
./quote-retrieval-mcp -transport http -addr :18080 &
...
curl -sN -X POST http://localhost:18080 \
  ...
  -d '{"jsonrpc":"2.0","id":2,"method":"tools/call","params":{"name":"quote-retrieval_load","arguments":{"entity":"quote","query":{"id":1}}}}'
```
