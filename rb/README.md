# QuoteRetrieval Ruby SDK



The Ruby SDK for the QuoteRetrieval API — an entity-oriented client using idiomatic Ruby conventions.
...
```ruby
client = QuoteRetrievalSDK.test(testopts, sdkopts)
```
...
raise a `QuoteRetrievalError` (a `StandardError` subclass), so wrap
...
```
rb/
├── QuoteRetrieval_sdk.rb       -- Main SDK module
...
The main module (`QuoteRetrieval_sdk`) exports the SDK class
