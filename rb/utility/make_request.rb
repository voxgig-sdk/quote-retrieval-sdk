...
module QuoteRetrievalUtilities
  MakeRequest = ->(ctx) {
    ...
    spec = ctx.spec
    utility = ctx.utility
    response = QuoteRetrievalResponse.new({})
    ...
    if fetch_err
      ...
      response = QuoteRetrievalResponse.new({ "err" => ctx.make_error("request_no_response", "response: undefined") })
    elsif fetched.is_a?(Hash)
      response = QuoteRetrievalResponse.new(fetched)
    else
