...
//     (\`new QuoteRetrievalSDK(...)\` / \`QuoteRetrievalSDK.test(...)\` become
//     \`QuoteRetrievalSDK.test({ entity: ... })\`), transpiled to JS, and run
...
const SDK_NAME = 'QuoteRetrievalSDK'
...
// rewrite real client construction (\`new QuoteRetrievalSDK(...)\` and any
// \`QuoteRetrievalSDK.test(...)\`) to a seeded test-mode client, and give
...
function rewriteForRun(code: string): string {
  ...
  out = out.replace(/new\s+QuoteRetrievalSDK\s*\([^)]*\)/g, () => SEEDED_CTOR)
  out = out.replace(/QuoteRetrievalSDK\.test\s*\([^)]*\)/g, () => SEEDED_CTOR)
