...
function envOverride(m: Record<string, any>) {
  if (
    'TRUE' === process.env.QUOTERETRIEVAL_TEST_LIVE ||
    'TRUE' === process.env.QUOTERETRIEVAL_TEST_OVERRIDE
  ) {
  ...
  m.QUOTERETRIEVAL_TEST_EXPLAIN = process.env.QUOTERETRIEVAL_TEST_EXPLAIN || m.QUOTERETRIEVAL_TEST_EXPLAIN
